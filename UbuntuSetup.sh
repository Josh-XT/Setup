...
ensure_lvm_tools() {
    if ! command -v lvs &> /dev/null || ! command -v pvs &> /dev/null || ! command -v vgs &> /dev/null; then
        ...
        if [ $? -ne 0 ]; then
            log_message "Failed to install LVM2 package. LVM-related operations will be skipped."
            return 1
        fi
        log_message "LVM2 package installed successfully."
    else
        log_message "LVM tools are already installed."
    fi
    return 0
}
...
lvm_pv=$(get_lvm_pv)
if [ $? -ne 0 ]; then
    log_message "Failed to get LVM physical volume. Skipping LVM expansion."
else
    log_message "LVM Physical Volume: $lvm_pv"

    # Print current LV size
    log_message "Current LV size:"
    sudo lvs --units g /dev/mapper/ubuntu--vg-ubuntu--lv
